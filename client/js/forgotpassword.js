...
let neonForgotPassword = neonForgotPassword || {};
...
;(function($, window, undefined) {
  ...
  $(document).ready(function() {
          ...
          neonForgotPassword.setPercentage(98, function() {
            ...
            $.ajax({
              ...
              success: function(response) {
                ...
                // We will give some time for the animation to finish,
    ...
    neonForgotPassword.$steps.find('[data-step]').on('click', function(ev) {
      ...
      let $current_step = neonForgotPassword.$steps_list.filter('.current');
      let next_step = $(this).data('step');
      let validator = neonForgotPassword.$container.data('validator');
      let errors = 0;
      ...
      if(errors) {
        ...
        let $next_step = neonForgotPassword.$steps_list.filter('#' + next_step);
        let $other_steps = neonForgotPassword.$steps_list.not( $next_step );

        let current_step_height = $current_step.data('height');
        let next_step_height = $next_step.data('height');
        ...
        TweenMax.to($current_step, .3, {css: {autoAlpha: 0}, onComplete: function() {
          ...
          let $form_elements = $next_step.find('.form-group');
    ...
    neonForgotPassword.$steps_list.each(function(i, el) {
      ...
      if(is_current) {
        ...
        $this.addClass('current').data('height', $this.outerHeight() + margin)
          .removeClass('current');
      }
    });
    ...
    neonForgotPassword.$login_progressbar =
        neonForgotPassword.$body.find('.login-progressbar div');
    ...
    $.extend(neonForgotPassword, {
      setPercentage: function(pct, callback) {
        ...
        let o = {
          pct: parseInt(neonForgotPassword.$login_progressbar.width()
            / neonForgotPassword.$login_progressbar.parent().width() * 100, 10),
        };
