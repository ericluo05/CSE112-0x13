...
function validatePhone() {
    ...
    var numOnly = /^\d/; // regex used to check if the phone number is numeric only
    ...
    if (numOnly.test(number.toString())) {
        xhttp.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200) {
                ...
                if (jsonRes['isValid']) result.innerHTML = 'Phone number is valid!';else result.innerHTML = 'Phone number is invalid!';
            }
        };
    } else {
...
function formatPhone() {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            var data = this.responseText;
            var jsonRes = JSON.parse(data);
            if (jsonRes['isValid']) result.innerHTML = 'Formatted Number: ' + jsonRes['Format'];else result.innerHTML = 'Your number is invalid!';
        }
    };
